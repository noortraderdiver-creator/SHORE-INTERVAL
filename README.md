...
2. Before clicking Deploy, add two **Environment Variables**:
   | Name | Value |
   |---|---|
...
**Still not real — same gaps as before, now more important to close:**
- **No real authentication.** "Joining" and "admin" are both open to anyone — there's no password or session tying a person to their applications, listings, or messages. This is the single biggest thing to fix before real users show up.
...
## Locking it down before real users show up

The `records` table currently has fully open policies — anyone with your Supabase anon key (which is visible in your deployed site's frontend, by design) can read and write everything. That's fine for a preview, not for production. Two things matter most, in order:

1. **Add real auth.** [Supabase Auth](https://supabase.com/docs/guides/auth) is free and built into the same project — email/password or magic-link sign-in. Once you have it, you can tie applications, listings, bookings, and messages to a real logged-in user instead of a name typed into a form.
2. **Tighten the database policies** so people can only write their own data — a host can edit their own listings but not someone else's, a member can see their own booking requests but not everyone's.
